...
sigma_a = 0.1532_real64               ! Sigma absorb
nusig = 0.1570_real64               ! Sigma fission*nu
D=3.62E-2_real64                    ! Sigma Transport
D = 1.0_real64/(3.0_real64*D)              ! Diffusion Coef
j_tot = 0
j_width=0
you=0
...
!--------------------------Matrix Declaration-----------------------------!
   ...
   A(1,1) = (0.5_real64*sigma_a)+(D/(dx**2))
   A(1,2) = -D/(dx**2)
   A(n,n) = sigma_a+2.0_real64*D/(dx**2)
   A(n,n-1) = -D/(dx**2)

   do i=2 , n-1

      A(i,i) = sigma_a+2.0_real64*D/(dx**2)
      A(i,i+1) = -D/(dx**2)
      A(i,i-1) = -D/(dx**2)
