...
real(real64)              :: w, ratio, dr1, dr2,holder
...
do p=1,g
   ...
   A(1,2,p) = -0.5_real64*sigma_tr(p,1)/(dr1)

   A(n_tot,n_tot,p) = sigma_a(1,2)*dr2 + ((0.5)-(1/i))*sigma_tr(p,2)/(dr2)
   !A(n_tot,n_tot-1,p) = -(1.0_real64-1.0_real64/(2.0_real64*real(n_tot,real64)-1.0_real64))*sigma_tr(p,2)/(dr2)

   A(n,n-1,p) =   sigma_tr(p,1)/(i*dr1*dr1)  
   A(n,n+1,p) =  -sigma_tr(p,2)/(i*dr2*dr2) 
    holder= (1.0-(1.0/i)) *sigma_tr(p,1)/(dr1)+(1.0-(1.0/i)-(1.0/(i*dr2)))*sigma_tr(p,2)/(dr2)
   A(n,n,p)=holder+(sigma_a(p,2)*dr2+sigma_a(p,1)*dr1)/2.0

   do i=2 , n-1
 
      A(i,i,p) = 0.5*sigma_r(p,1) *dr1 +(1.0-(1/i)+(1/i*dr1))*sigma_tr(p,1)/(dr1)
      A(i,i+1,p) =-0.5_real64*sigma_tr(p,1)/(dr1)
      A(i,i-1,p) =(0.5+(1.0/(i*dr1)))*sigma_tr(p,1)/(-dr1)

   enddo
   do i=n+1 , n_tot-1

      A(i,i,p) = 0.5*sigma_a(p,2)*dr2+(1.0-(1/i)+(1/i*dr2))*sigma_tr(p,2)/(dr2)
      A(i,i+1,p) = -0.5_real64*sigma_tr(p,2)/(dr2)
      A(i,i-1,p) = -(0.5+(1.0/(i*dr1)))*sigma_tr(p,2)/(dr2)
